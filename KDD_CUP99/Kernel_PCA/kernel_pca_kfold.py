...
dataset = pd.read_csv('kdd_Overall.csv')
...
kpca = KernelPCA(kernel="rbf", fit_inverse_transform=True, gamma=10,n_components=5)
...
#Applying Kfold Algorithm

from sklearn.model_selection import cross_val_score
accur=cross_val_score(estimator=classifier,X=X_train,y=y_train,cv=10)
M=accur.mean()
N=accur.std()
...
print('\nPrecision : %f' %(fnr))
print('\nRecall : %f' %(fdr))
print('\nAccuracy using Kfold Algorithm : %f' %(M))
